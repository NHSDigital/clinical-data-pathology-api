...
Select the Bruno environment you want to use for the collection, there may be multiple environments for a collection, this will be defined as part of the code in the collection.

![environment selection](assets/bruno_guide/environment.png)

Some of the environment variables will be hidden and not committed to the repository, for these values you will need to populate them yourself.

Some collections may have custom scripts to run before/after requests, if these script require external dependencies then you will need to install these using `npm install` in the directory of the collection. For requests using external libraries or using the file system you will need to make sure Bruno is running in developer mode.
